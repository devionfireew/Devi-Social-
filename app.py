                    ...
                    if not all([fn, un, em, pw, ct, bio]):
                        ...
                        if res["ok"]:
                            data = {
                                ...
                                "birthday": bd.isoformat(), "city": ct, "bio": bio,
...
if page == "Feed":
    ...
    for post in db.collection("posts").order_by("timestamp", direction=firestore.Query.DESCENDING).limit(50).get():
        ...
        # Content
        if p.get("content"):
            st.markdown(f"<p style='font-size:1.1rem;'>{p['content']}</p>", unsafe_allow_html=True)
            ...
            if q.lower() in d.get("username","").lower() or q.lower() in d.get("full_name","").lower() or q.lower() in d.get("city","").lower():
                ...
                with cols[1]:
                    ...
                    if status == "none":
                        ...
                        c1.markdown("<p style='color:#00ff7f;'>✨ Friends</p>", unsafe_allow_html=True)
                    elif status == "pending_in":
                        c1.markdown("<p style='color:#ffa500;'>⏳ Requested</p>", unsafe_allow_html=True)
                    elif status == "pending_out":
                        c1.markdown("<p style='color:#ffa500;'>⏳ Pending</p>", unsafe_allow_html=True)
...
# ========================================
    ...
    with c2:
        if not st.session_state.chat_user:
            ...
            with chat_box:
                if not msgs:
                    st.markdown("<p style='text-align:center; color:#666;'>No messages yet. Say hello! 👋</p>", unsafe_allow_html=True)
                ...
                for m in msgs:
                    ...
                    st.markdown(f"""
                        <div style='text-align:{align}; margin:8px 0; overflow:hidden;'>
                            <div class='{cls}'>{m['message_body']}</div>
                            <div style='clear:both;'></div>
                            <div style='font-size:0.7rem; color:#555; text-align:{align};'>{tm}</div>
                        </div>
                    """, unsafe_allow_html=True)
...
# ========================================
    ...
    with c1:
        ...
        if pic:
            ...
            st.markdown("<div style='width:120px;height:120px;border-radius:50%;background:#333;display:flex;align-items:center;justify-content:center;font-size:3rem;margin:auto;'>👤</div>", unsafe_allow_html=True)
    ...
    with c3:
        if is_me:
            ...
            if status == "none":
                ...
                st.markdown("<p style='color:#ffa500;'>⏳ They sent you a request</p>", unsafe_allow_html=True)
            elif status == "pending_out":
                st.markdown("<p style='color:#ffa500;'>⏳ Request sent</p>", unsafe_allow_html=True)
    
    # User's Posts
    st.markdown("---")
    st.subheader("Posts")
    
    posts = list(db.collection("posts").where("author_id", "==", uid).order_by("timestamp", direction=firestore.Query.DESCENDING).limit(20).get())
    if not posts:
            ...
            if st.form_submit_button("Send") and p.strip():
                ...
                try:
                    ...
                    resp = client.chat.completions.create(
                        ...
                        messages=[{"role": "system", "content": "You are Devi AI, a helpful assistant."}] + 
